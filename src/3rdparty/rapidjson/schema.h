	...
	public:
			...
			exclusiveMaximum_(false)
		{
			...
			if(const ValueType* v = GetMember(value, GetPatternPropertiesString()))
			{
				...
				patternProperties_ = static_cast<PatternProperty*>(allocator_->Malloc(sizeof(PatternProperty) *
				                     v->MemberCount()));
				...
				for(ConstMemberIterator itr = v->MemberBegin(); itr != v->MemberEnd(); ++itr)
				{
					...
					schemaDocument->CreateSchema(&patternProperties_[patternPropertyCount_].schema, q.Append(itr->name,
...
foundAny:
		...
		bool StartObject(Context & context) const
		{
			...
			if(patternProperties_)    // pre-allocate schema array
			{
				...
				context.patternPropertiesSchemas = static_cast<const SchemaType**>(context.factory.MallocState(sizeof(
...
#define RAPIDJSON_STRING_(name, ...) \
	static const ValueType& Get##name##String() {\
		...
		RAPIDJSON_STRING_(PatternProperties, 'p', 'a', 't', 't', 'e', 'r', 'n', 'P', 'r', 'o', 'p', 'e', 'r', 't', 'i',
		                  'e', 's')
		RAPIDJSON_STRING_(AdditionalProperties, 'a', 'd', 'd', 'i', 't', 'i', 'o', 'n', 'a', 'l', 'P', 'r', 'o', 'p',
		                  'e', 'r', 't', 'i', 'e', 's')
		...
		RAPIDJSON_STRING_(ExclusiveMinimum, 'e', 'x', 'c', 'l', 'u', 's', 'i', 'v', 'e', 'M', 'i', 'n', 'i', 'm', 'u',
		                  'm')
		RAPIDJSON_STRING_(ExclusiveMaximum, 'e', 'x', 'c', 'l', 'u', 's', 'i', 'v', 'e', 'M', 'a', 'x', 'i', 'm', 'u',
		                  'm')
	...
	private:
			...
			if(value.IsString())
				try
				{
					return new(allocator_->Malloc(sizeof(RegexType))) RegexType(value.GetString(),
					        std::size_t(value.GetStringLength()), std::regex_constants::ECMAScript);
				}
		...
		bool CreateParallelValidator(Context & context) const
		{
			...
			if(validatorCount_)
			{
				...
				context.validators = static_cast<ISchemaValidator**>(context.factory.MallocState(sizeof(
	...
	struct TokenHelper
	{
		RAPIDJSON_FORCEINLINE static void AppendIndexToken(Stack & documentStack, SizeType index)
		{
			*documentStack.template Push<Ch>() = '/';
			char buffer[21];
			size_t length = static_cast<size_t>((sizeof(SizeType) == 4 ? u32toa(index, buffer) : u64toa(index,
			                                     buffer)) - buffer);
		...
		*documentStack_.template Push<Ch>() = '\0';
		...
		if(!schemaStack_.Empty())
		{
			...
			if(context.valueUniqueness)
			{
				...
				if(!a)
				{
					CurrentContext().arrayElementHashCodes = a = new(GetStateAllocator().Malloc(sizeof(
					            HashCodeArray))) HashCodeArray(kArrayType);
				}
...
public:
	...
	*/
	SchemaValidatingReader(InputStream & is, const SchemaDocumentType & sd) : is_(is), sd_(sd),
		invalidSchemaKeyword_(), isValid_(true) {}
