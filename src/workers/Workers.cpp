...
void Workers::printHashrate(bool detail) { m_hashrate->print(); }
...
void Workers::stop()
{
    ...
    m_paused   = 0;
    m_sequence = 0;
    for (size_t i = 0; i < m_workers.size(); ++i) { m_workers[i]->join(); }
}
...
void Workers::onReady(void *arg)
{
    ...
    if (Mem::isDoubleHash()) { handle->setWorker(new DoubleWorker(handle)); }
    else { handle->setWorker(new SingleWorker(handle)); }
    handle->worker()->start();
}
void Workers::onResult(uv_async_t *handle)
{
    ...
    uv_mutex_unlock(&m_mutex);
    for (auto result : results) { m_listener->onJobResult(result); }
    results.clear();
}
void Workers::onTick(uv_timer_t *handle)
{
    ...
    if ((m_ticks++ & 0xF) == 0)  { m_hashrate->updateHighest(); }
#   ifndef XMRIG_NO_API
    Api::tick(m_hashrate);
#   endif
}