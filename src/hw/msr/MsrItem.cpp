...
xmlcore::MsrItem::MsrItem(const rapidjson::Value &value)
{
    if (!value.IsString()) {
        return;
    }
...
rapidjson::Value xmlcore::MsrItem::toJSON(rapidjson::Document &doc) const
{
    return toString().toJSON(doc);
}


xmlcore::String xmlcore::MsrItem::toString() const
{
