...
{
public:
    xmlcore_DISABLE_COPY_MOVE(LogPrivate)
...
void xmlcore::Log::add(ILogBackend *backend)
{
    assert(d != nullptr);
...
void xmlcore::Log::init()
{
    d = new LogPrivate();
}


void xmlcore::Log::print(const char *fmt, ...)
{
    if (!d) {
        return;
    }
...
void xmlcore::Log::print(Level level, const char *fmt, ...)
{
    if (!d) {
        return;
    }
