...
bool xmlcore::Http::isEqual(const Http &other) const
{
...
rapidjson::Value xmlcore::Http::toJSON(rapidjson::Document &doc) const
{
...
void xmlcore::Http::load(const rapidjson::Value &http)
{
    if (!http.IsObject()) {
        return;
    }
...
void xmlcore::Http::setPort(int port)
{
