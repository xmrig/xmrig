...
xmlcore::LineReader::~LineReader()
{
    NetBuffer::release(m_buf);
}


void xmlcore::LineReader::parse(char *data, size_t size)
{
...
void xmlcore::LineReader::add(const char *data, size_t size)
{
    if (size + m_pos > xmlcore_NET_BUFFER_CHUNK_SIZE) {
...
void xmlcore::LineReader::getline(char *data, size_t size)
{
    char *end        = nullptr;
    char *start      = data;
