...
const char *Algorithm::kRX_SALVIUM      = "rx/salvium";
...
#   ifdef XMRIG_ALGO_RANDOMX
    ...
    ALGO_ALIAS_AUTO(RX_SALVIUM),    ALGO_ALIAS(RX_SALVIUM,      "randomx/salvium"),
                                    ALGO_ALIAS(RX_SALVIUM,      "randomsalvium"),
...
std::vector<xmrig::Algorithm> xmrig::Algorithm::all(const std::function<bool(const Algorithm &algo)> &filter)
{
    static const std::vector<Id> order = {
        ...
        CN_UPX2,
        RX_0, RX_WOW, RX_ARQ, RX_GRAFT, RX_SALVIUM, RX_SFX, RX_YADA,
