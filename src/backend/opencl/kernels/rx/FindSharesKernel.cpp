...
void xmlcore::FindSharesKernel::enqueue(cl_command_queue queue, size_t threads)
{
...
void xmlcore::FindSharesKernel::setArgs(cl_mem hashes, cl_mem shares)
{
...
void xmlcore::FindSharesKernel::setTarget(uint64_t target)
{
    setArg(1, sizeof(uint64_t), &target);
}


void xmlcore::FindSharesKernel::setNonce(uint32_t nonce)
{
