...
void xmrig::OclRxJitRunner::init()
{
    ...
    m_registers             = createSubBuffer(CL_MEM_READ_WRITE, 256 * m_intensity);
    m_intermediate_programs = createSubBuffer(CL_MEM_READ_WRITE, 5120 * m_intensity);
    m_programs              = createSubBuffer(CL_MEM_READ_WRITE, 10048 * m_intensity);
}
