...
xmlcore::OclSharedData &xmlcore::OclSharedState::get(uint32_t index)
{
    return map[index];
}


void xmlcore::OclSharedState::release()
{
    for (auto &kv : map) {
        kv.second.release();
    }
...
void xmlcore::OclSharedState::start(const std::vector<OclLaunchData> &threads, const Job &job)
{
    assert(map.empty());
