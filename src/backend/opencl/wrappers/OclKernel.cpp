...
xmlcore::OclKernel::OclKernel(cl_program program, const char *name) :
    m_name(name)
{
...
xmlcore::OclKernel::~OclKernel()
{
    OclLib::release(m_kernel);
}


void xmlcore::OclKernel::enqueueNDRange(cl_command_queue queue, uint32_t work_dim, const size_t *global_work_offset, const size_t *global_work_size, const size_t *local_work_size)
{
...
void xmlcore::OclKernel::setArg(uint32_t index, size_t size, const void *value)
{
