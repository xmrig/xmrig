...
bool xmlcore::RxConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
rapidjson::Value xmlcore::RxConfig::toJSON(rapidjson::Document &doc) const
{
...
#ifdef xmlcore_FEATURE_HWLOC
std::vector<uint32_t> xmlcore::RxConfig::nodeset() const
{
    if (!m_nodeset.empty()) {
        return m_nodeset;
    }
...
const char *xmlcore::RxConfig::modeName() const
{
    return modeNames[m_mode];
}


uint32_t xmlcore::RxConfig::threads(uint32_t limit) const
{
    if (m_threads > 0) {
        return m_threads;
    }
...
#ifdef xmlcore_FEATURE_MSR
const char *xmlcore::RxConfig::msrPresetName() const
{
    return modNames[msrMod()];
}


const xmlcore::MsrItems &xmlcore::RxConfig::msrPreset() const
{
...
uint32_t xmlcore::RxConfig::msrMod() const
{
    if (!wrmsr()) {
...
void xmlcore::RxConfig::readMSR(const rapidjson::Value &value)
{
    if (value.IsBool()) {
...
xmlcore::RxConfig::Mode xmlcore::RxConfig::readMode(const rapidjson::Value &value) const
{
    if (value.IsUint()) {
