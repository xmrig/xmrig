...
if (CMAKE_SYSTEM_PROCESSOR MATCHES "^(ppc64le|powerpc64le)$")
    set(XMRIG_PPC ON)
    set(XMRIG_PPC_BITS 64)
    add_definitions(-DXMRIG_PPC -DXMRIG_PPC_BITS=64)
    message(STATUS "Detected PowerPC 64-bit little-endian architecture (${CMAKE_SYSTEM_PROCESSOR})")
elseif (CMAKE_SYSTEM_PROCESSOR MATCHES "^(ppc64|powerpc64)$")
    set(XMRIG_PPC ON)
    set(XMRIG_PPC_BITS 64)
    add_definitions(-DXMRIG_PPC -DXMRIG_PPC_BITS=64)
    message(STATUS "Detected PowerPC 64-bit big-endian architecture (${CMAKE_SYSTEM_PROCESSOR})")
elseif (CMAKE_SYSTEM_PROCESSOR MATCHES "^(ppc|powerpc|ppc32|powerpc32)$")
    set(XMRIG_PPC ON)
    set(XMRIG_PPC_BITS 32)
    add_definitions(-DXMRIG_PPC -DXMRIG_PPC_BITS=32)
    message(STATUS "Detected PowerPC 32-bit big-endian architecture (${CMAKE_SYSTEM_PROCESSOR})")
endif()
...
# Disable x86/ARM-specific features for PowerPC
if (XMRIG_PPC)
    set(WITH_SSE4_1 OFF)
    set(WITH_AVX2 OFF)
    set(WITH_VAES OFF)
    ...
    # GCC may emit libatomic calls for 64-bit atomics on PowerPC targets
    find_library(ATOMIC_LIBRARY NAMES atomic libatomic.so.1)
    if (ATOMIC_LIBRARY)
        set(XMRIG_PPC_LIBS ${ATOMIC_LIBRARY})
        message(STATUS "Linking libatomic for PowerPC: ${ATOMIC_LIBRARY}")
    endif()
endif()
